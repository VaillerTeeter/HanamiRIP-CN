...
import type { ItemStatus, StatusKey } from "../types/tracking";
import type { UseQueryPageReturn } from "../composables/useQueryPage";
import { formatAirDate, formatRating, formatStars } from "../utils/format";
        ...
        <NCard title="选择查询条件" size="small">
          <NSpace align="center" size="medium" class="query-actions">
            <NSelect
              ...
              placeholder="选择年份"
              @update:value="(value) => (query.year.value = value as number | null)"
            />
            <NSelect
              :value="query.month.value"
              :options="query.seasonOptions.value"
              placeholder="选择季度"
              @update:value="(value) => (query.month.value = value as number | null)"
            />
          ...
          <NCard title="条目列表" size="small" class="result-list">
            ...
            >
              <div
                ...
                role="button"
                tabindex="0"
                :ref="(el) => query.setListItemRef(item, el as HTMLElement | null)"
